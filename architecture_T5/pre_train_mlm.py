...
# ... (Tutte le funzioni di decodifica, parse_rdf, run_validation, get_ds, get_model rimangono qui) ...
# (Le copio per completezza, non ci sono modifiche)
...
def greedy_decode(model, source, source_mask, tokenizer, max_len, device):
    sot_idx, eot_idx = tokenizer.token_to_id('<SOT>'), tokenizer.token_to_id('<EOT>')
    encoder_output = model.encode(source, source_mask)
    decoder_input = torch.empty(1, 1).fill_(sot_idx).type_as(source).to(device)
    while decoder_input.size(1) < max_len:
        out = model.decode(encoder_output, source_mask, decoder_input, None)
...
def beam_search_decode(model, beam_size, source, source_mask, tokenizer, max_len, device):
    ...
    for _ in range(max_len - 1):
        ...
        for candidate_seq, candidate_score in beams:
            ...
            out = model.decode(encoder_output, source_mask, candidate_seq, None)
...
def train_model(config, phase: str):
    ...
    # Per il pre-training MLM, disabilitiamo il label smoothing per convergere più velocemente
    if phase == 'pretrain':
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            writer.add_scalar(f'train_loss_step/{phase}', loss.item(), global_step)
            writer.add_scalar(f'learning_rate/{phase}', optimizer.param_groups[0]['lr'], global_step)
            loss.backward();
        ...
        writer.add_scalar(f'train_loss_epoch/{phase}', avg_epoch_loss, epoch)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train the NanoSocrates model.')
